...
<html>
    ...
    <body>
        ...
        <div style="padding: 20px;">
            ...
            <%-- Live Status Board --%>
            <div class="queue-status-board" style="background: #f4f4f4; padding: 15px; border-radius: 8px; margin-bottom: 20px; border: 1px solid #ddd;">
                <h3 style="margin-top: 0;">Live Queue Status</h3>
                <div style="display: flex; gap: 40px;">
                    <div style="text-align: center;">
                        <p style="margin: 0; color: #666;">Current Progress</p>
                        <h2 id="currentProgressDisplay" style="color: #2ecc71; font-size: 2.5em; margin: 10px 0;">${currentProgress != null ? currentProgress : '--'}</h2>
                    </div>
                    <div style="text-align: center;">
                        <p style="margin: 0; color: #666;">Latest Number Issued</p>
                        <h2 id="latestJoinedDisplay" style="color: #3498db; font-size: 2.5em; margin: 10px 0;">${latestJoined != null ? latestJoined : '--'}</h2>
                    </div>
                </div>
                <small id="statusDetail" style="color: #888;">* Select a clinic and service to see live numbers.</small>
            </div>
            ...
            <form id="walkinForm" action="handlePatient" method="POST">
                ...
                <input type="submit" value="Get Queue Number" style="padding: 10px 20px; background: #3498db; color: white; border: none; border-radius: 4px; cursor: pointer;">
            </form>

            <p style="margin-top: 20px;"><a href="handlePatient?action=profile">Back to Profile</a></p>
        </div>
                    ...
                    if (clinicId && serviceId) {
                        $.ajax({
                            ...
                            success: function (data) {
                                $('#currentProgressDisplay').text(data.current);
                                $('#latestJoinedDisplay').text(data.latest);
                                $('#statusDetail').text("* Showing status for " + serviceName + " in " + clinicName) ;
                            },
                            error: function () {
                                console.log("AJAX Error: Could not fetch status.");
                            }
                        });
                    }
                }
